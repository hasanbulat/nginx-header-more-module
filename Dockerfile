...
RUN install_packages wget build-essential zip libpcre3-dev zlib1g-dev libssl-dev libmaxminddb-dev libgeoip-dev
...
RUN cd /tmp/nginx-${NGINX_VERSION} && \
    ...
    --with-http_geoip_module --add-module=nginx-module-vts-${VTS_VERSION} --add-dynamic-module=ngx_http_geoip2_module-${GOIP2_VERSION} --add-module=ngx_http_substitutions_filter_module-${SUBTITUTIONS_FILTER_VERSION} && \
    make && \
    make install
